package com.yukta.systemdesign.lld.multithreading.core_threading.lock.reenterant_lock;
...
class TicketBookingTryLock {
    ...
    private static final Log log = LogFactory.getLog(TicketBookingTryLock.class);
    ...
    public void bookTicket(String user) {
        log.info(user + " is trying to book...");
        ...
        try {
            ...
            if (lockAcquired) {
                log.info(user + " acquired lock.");

                // critical section – safe to inspect/update seats
                if (availableSeats > 0) {
                    log.info(user + " successfully booked the ticket.");
                    availableSeats--;
                } else {
                    log.info(user + " could not book the ticket. No seats left.");
                }
                ...
                log.info(user + " could not acquire lock. Try again later.");
            }
        }
            ...
            if (lockAcquired) {
                log.info(user + " is releasing the lock.");
                lock.unlock();
            }
        }
    }
